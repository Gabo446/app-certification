...
export class DocumentService {
  ...
  async saveDocument(fileName: string, htmlContent: string, fileSize: number): Promise<string> {
  ...
  async getUserDocuments(): Promise<DocumentRecord[]> {
    ...
    const user: User | null = await new Promise((resolve) => {
      const unsubscribe = onAuthStateChanged(auth, (user) => {
        unsubscribe();
        resolve(user); // user es de tipo User | null
      });
    });
    ...
    // Ahora TypeScript sabe que `user` es de tipo User → tiene `.uid`
    const q = query(
      collection(this.firestore, this.collectionName),
      where('userId', '==', user.uid) // ✅ Ahora sí existe `uid`
    );
